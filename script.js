...
const API_KEY = `9e72cc93680145b5a3d90f62a051dd28`;
const fixedCuisines = [
   "Asian", "American","Italian","Middle Eastern",
];
...
const receiveRecipeData = () => {
  fetch(URL)
    .then(response => {
      if (!response.ok) throw new Error("Something whent wrong");
    ...
    .catch(error => {
      alert("There is an issue with retrieving your recipes,\n please try again or continue with a new search word.\n\n " + error);
...
// Get unique cuisine filter values
...
const generateButtons = (container, recipes, type) => {
  ...
  container.innerHTML = `
 <button "${type === "filter" ? "Show all cuisines" : "Reset sorting"}" value="all" class="${buttonClass}">
      ${buttonText}
    </button>
...
const FindRecipe = (recipes) => {
  ...
  if (query === "") {
    ...
    recipeContainer.innerHTML = `<p>No matching recipes found, try something else?</p>`;
  } else {
    fillRecipeCards(filteredRecipes);
  }
}